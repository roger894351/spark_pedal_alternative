...
- **🎲 Random** (key `R`): builds a random amp + effects combination and sends it, for auditioning ideas. Hendrix (J.H.) gear is included by default and can be switched off under **My tones** if you don't own that pack. Volume follows the previous tone. Tap **Undo** to go back, or **Save current tone** to keep it.
...
| Old version still showing                | The log header should say `v16`; pull down to reload in Bluefy          |
