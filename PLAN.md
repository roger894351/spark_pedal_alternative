...
- [x] **Flow control + ack accounting** (v11). Multi-block writes use `writeValue` with a 30 ms
      ...
      once. Done.
- [ ] **Verify after write.** The app already knows the tone it believes is playing. 1.5 s after
...
- [ ] **Signal chain view.** The 7 slots left→right (Gate → Comp → Drive → Amp → Mod → Delay →
...
- [ ] **A/B compare** on `Space` — flip between the current tone and the one you started from.
      This is the single most useful missing key when auditioning random tones.
- [ ] **More keys:** `Q W E R T Y U` toggle the 7 slots like stompboxes; `Shift`+digit saves the
